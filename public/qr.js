// ============ QRコード生成（招待リンクを見せるためだけの最小実装） ============
// 外部ライブラリを足したくないので自前で持つ。用途は「招待リンクをその場で見せる」ことだけ。
//
// バージョン5・誤り訂正L（37×37・最大106バイト）に固定してある。招待リンクは
// 長くても70文字程度なので足りる。固定にするとブロック分割もバージョン情報パターンも
// 要らなくなり、実装がぐっと短くなる（V7以上だとどちらも必要になる）。
...
export function qrSVG(text, cls = "qr-svg") {
  ...
  return `<svg class="${cls}" viewBox="0 0 ${side} ${side}" shape-rendering="crispEdges" role="img" aria-label="招待リンクのQRコード">
